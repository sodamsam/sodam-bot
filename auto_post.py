...
APPLY_CTA_LINES = [
    "바로 쓰는 AI 프롬프트 5종 모음, 댓글에 '신청' 남겨주시면 보내드려요",
    "업무에 바로 쓰는 프롬프트 모음집 나눔 중이에요. 댓글에 '신청'이라고 적어주세요",
    "정리해둔 프롬프트 5종, 댓글에 '신청' 남기시면 보내드립니다",
]
...
def write_prompt_post(state, now_kst):
    ...
    area = AREAS[seq % 3]
    bank = PROMPT_BANK[area]
    idx = (seq // 3) % len(bank)
    topic, keyword = bank[idx]
